...
const [home, bootCamp, howItWorks, pricing, trial, lesson, prototype, robots] = await Promise.all([
...
// Missing media caused earlier recovery regressions: verify local media in
// both review routes, rather than only the generated HTML.
for (const html of [lesson, prototype]) {
  for (const match of html.matchAll(/src="(\/media\/[^"?#]+)(?:[?#][^"]*)?"/g)) {
