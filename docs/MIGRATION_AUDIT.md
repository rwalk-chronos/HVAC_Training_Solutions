...
- The original inventory has **21 unresolved published page paths**, including seven checkout/payment/enrollment paths, old webinar and signup flows, three brazing lesson pages, a thermistor page, and clone/test pages. PR #11 now serves the existing `/hvac-boot-camp/course-purchase/` thank-you path in development, but the PayPal return setting is not verified; keep that row open for return QA. Ron manually verifies payment and enrolls students in Moodle. The `next_action` column distinguishes payment blockers from technical content and retirement review. These are explicit migration decisions, not a public-cutover-ready URL map. Do not redirect the old WordPress checkout paths until their treatment is verified.
...
1. Finish classifying the original 21 unresolved published pages, including testing the now-built thank-you path as each PayPal button's return destination; keep the current WordPress checkout pages live until both PayPal plans and Ron's manual Moodle handoff are tested.
