...
- The inventory has **20 unresolved published page paths**, including six checkout/payment/enrollment paths, old webinar and signup flows, three brazing lesson pages, a thermistor page, and clone/test pages. PR #11 serves the existing `/hvac-boot-camp/course-purchase/` thank-you path in development; its row is marked `built_pending_return_test` because both PayPal return settings and the manual Moodle handoff still need verification. Ron manually verifies payment and enrolls students in Moodle. The `next_action` column distinguishes payment blockers from technical content and retirement review. These are explicit migration decisions, not a public-cutover-ready URL map. Do not redirect the old WordPress checkout paths until their treatment is verified.
...
1. Finish classifying the 20 unresolved published pages and test the built thank-you path as each PayPal button's return destination; keep the current WordPress checkout pages live until both PayPal plans and Ron's manual Moodle handoff are tested.
